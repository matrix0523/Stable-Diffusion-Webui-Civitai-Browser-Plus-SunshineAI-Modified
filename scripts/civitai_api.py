...
def create_api_url(content_type=None, sort_type=None, period_type=None, use_search_term=None, base_filter=None, only_liked=None, tile_count=None, search_term=None, nsfw=None, isNext=None):
    base_url = "https://www.civitai.work/api/v1/models"
    version_url = "https://www.civitai.work/api/v1/model-versions"
    ...
    if use_search_term != "None" and search_term:
        ...
        if "www.civitai.work" in search_term:
    ...
    if model_id:
        ...
        for item in api_data['items']:
            if int(item['id']) == int(model_id):
                ...
                model_main_url = f"https://www.civitai.work/models/{item['id']}"
                img_html = '<div class="sampleimgs"><input type="radio" name="zoomRadio" id="resetZoom" class="zoom-radio" checked>'
                
                url = f"https://www.civitai.work/api/v1/model-versions/{selected_version['id']}"
                ...
                if not creator or model_uploader == 'User not found':
                    ...
                    uploader = f'<h3 class="model-uploader">Uploaded by <a href="https://www.civitai.work/user/{escape(str(model_uploader))}" target="_blank">{escape(str(model_uploader))}</a>{uploader_avatar}</h3>'
                output_html = f'''
...
def get_headers(referer=None, no_api=None):
    ...
    if referer:
        headers['Referer'] = f"https://www.civitai.work/models/{referer}"
